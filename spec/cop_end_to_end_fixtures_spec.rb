...
RSpec.describe "cop-owned end-to-end fixtures" do
  fixture_root = File.join(ROOT, "spec", "fixtures", "cops")
  fixture_directories = Dir.glob(File.join(fixture_root, "*", "*", "end_to_end", "*"))
  ...
  fixture_directories.sort.each do |fixture_directory|
    ...
    cop = fixture_directory.delete_prefix("#{fixture_root}/").split(File::SEPARATOR).first(2).join("/")

    it "matches RuboCop for #{cop} in #{fixture_name}", cop: cop do
  ...
  Dir.glob(File.join(fixture_root, "*", "*", "end_to_end", "*", "source.yml")).sort.each do |source_path|
    fixture_directory = File.dirname(source_path)
    fixture_name = File.basename(fixture_directory)
    cop = fixture_directory.delete_prefix("#{fixture_root}/").split(File::SEPARATOR).first(2).join("/")

    it "matches RuboCop diagnostics and corrections for sourced #{cop} fixture #{fixture_name}", cop: cop do
