...
fn check_rails(path: &str, lines: &[SourceLine], options: &Options, offenses: &mut Vec<Offense>) {
    for (index, line) in lines.iter().enumerate() {
        let text = strip_comment(&line.body);

        if cop_enabled(options, "Rails/DefaultScope") && text.contains("default_scope") {
            push_offense(
                offenses,
                "Rails/DefaultScope",
        ...
        if cop_enabled(options, "Rails/FilePath")
        ...
        if cop_enabled(options, "Rails/ApplicationJob")
        ...
        if cop_enabled(options, "Rails/ReversibleMigration")
...
fn check_rspec(path: &str, lines: &[SourceLine], options: &Options, offenses: &mut Vec<Offense>) {
    ...
    for (index, line) in lines.iter().enumerate() {
        ...
        if cop_enabled(options, "RSpec/Focus")
        ...
        if is_rspec_group_start(trimmed) {
            ...
            if cop_enabled(options, "RSpec/NestedGroups") && depth > 4 {
                push_offense(
                    offenses,
                    "RSpec/NestedGroups",
        ...
        } else if cop_enabled(options, "RSpec/MessageChain")
        ...
        } else if cop_enabled(options, "RSpec/PendingWithoutReason")
            ...
            || trimmed.starts_with("around")
        {
            if let Some(group) = group_stack.last_mut() {
                ...
                if cop_enabled(options, "RSpec/ScatteredSetup") && group.setup_blocks > 1 {
                    push_offense(
                        offenses,
            ...
            if let Some(group) = group_stack.last_mut() {
                ...
                if cop_enabled(options, "RSpec/MultipleMemoizedHelpers")
            ...
            if let Some(name) = symbol_argument(trimmed) {
                if cop_enabled(options, "RSpec/VariableName") && !is_snake_case(name) {
                    push_offense(
                        offenses,
                        "RSpec/VariableName",
        ...
        if trimmed == "end" {
            if let Some(example) = example_stack.pop() {
                ...
                if cop_enabled(options, "RSpec/ExampleLength") && length > 15 {
                    push_offense(
                        offenses,
                        "RSpec/ExampleLength",
                ...
                if cop_enabled(options, "RSpec/MultipleExpectations") && expectations > 1 {
                    push_offense(
                        offenses,
            ...
            if let Some(group) = group_stack.pop() {
                if cop_enabled(options, "RSpec/EmptyExampleGroup") && group.examples == 0 {
                    push_offense(
                        offenses,
...
fn check_rspec_file_path(path: &str, options: &Options, offenses: &mut Vec<Offense>) {
    if cop_enabled(options, "RSpec/SpecFilePathSuffix") && !path.ends_with("_spec.rb") {
        push_offense(
            offenses,
    ...
    if cop_enabled(options, "RSpec/SpecFilePathFormat")
        && path.contains("/spec/")
        && path.contains("__")
    {
        push_offense(
