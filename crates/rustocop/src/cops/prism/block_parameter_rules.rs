...
define_cops! {
    ...
    NumberedParameters => "Style/NumberedParameters" => node(as_block_node, numbered_parameters),
    NumberedParametersLimit => "Style/NumberedParametersLimit" => node(as_block_node, numbered_parameters_limit),
}

fn numbered_parameters(node: &ruby_prism::BlockNode<'_>, context: &mut CopContext<'_, '_>) {
    ...
    let multiline = context.source_file().at(&node.location()).contains('\n');
    ...
    if style == "disallow" {
        context.report(
            ...
            parent.map_or_else(|| node.location(), Node::location),
        );
    } else {
        context.report(
