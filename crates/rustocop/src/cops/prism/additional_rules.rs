...
fn empty_heredoc(source: &str, reporter: &mut Reporter<'_>) {
    for (offset, line) in source_lines(source) {
...
fn space_inside_range(source: &str, reporter: &mut Reporter<'_>) {
    ...
    for (offset, line) in source_lines(source) {
        ...
        let dot_count = if line[dots..].starts_with("...") {
            3
        } else {
            2
        };
...
fn space_after_method_name(source: &str, reporter: &mut Reporter<'_>) {
    for (offset, line) in source_lines(source) {
        let trimmed = line.trim_start();
...
fn redundant_constant_base(source: &str, reporter: &mut Reporter<'_>) {
