...
fn top_level_entries(source: &str) -> Vec<(usize, &str)> {
...
fn matching_delimiter(source: &str, open: u8, close: u8) -> Option<usize> {
    let mut depth = 0usize;
...
fn returns_after_continuation(lines: &[(usize, &str)], indent: usize) -> bool {
    for (_, line) in lines {
