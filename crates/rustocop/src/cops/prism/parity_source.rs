...
fn variable_interpolation(context: &mut CopContext<'_, '_>) {
    let source = context.source();
...
fn percent_word_literal(source: &str, offset: usize) -> bool {
    let line_start = source[..offset].rfind('\n').map_or(0, |at| at + 1);
    source[line_start..offset].contains("%W[") || source[line_start..offset].contains("%I[")
}

fn single_quoted_ranges(source: &str) -> Vec<std::ops::Range<usize>> {
...
fn unmatched_closing_brace(content: &str) -> bool {
    let mut depth = 0_i32;
